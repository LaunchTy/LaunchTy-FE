...
import { convertNumToOnChainFormat } from "./decimal";
...
export async function deployContract(
	...
	const artifactsPath = "src/app/abi/".concat(contractName, ".json");
	...
	if (
		...
		if (nameInConfig === "MockVDot") {
			let vAsset: any = (
				chainConfigJSON[31337]["vAssets"] as object[]
	...
	writeFileSync(
		"src/app/config/chainConfig.json",
		JSON.stringify(chainConfigJSON, null, 2)
	);
	console.log(
		"\x1b[32m%s\x1b[0m",
		`Saved ${nameInConfig} contract address to src/app/config/chainConfig.json`
	);
	return contract;
}
...
async function run(): Promise<void> {
	...
	// }
	// const mockVDot = await deployContract(
	// 	//launchpool
	// 	"MockVDot",
	// 	signer,
	// 	undefined
	...
	const launchpadFactoryContract = await deployContract(
		"LaunchpadFactory",
		signer,
		undefined
		// ethers.constants.AddressZero,
	);
	const launchpadFactoryAddress = launchpadFactoryContract.address;
	...
	console.log(
		"\x1b[36m%s\x1b[0m",
		`PoolFactory contract deployed to ${launchpadFactoryAddress}`
	);
	// console.log(
	// 	"\x1b[36m%s\x1b[0m",
	// 	`PreMarketFactory contract deployed to ${preMarketFactoryAddr}`
	// );
	// // console.log('\x1b[36m%s\x1b[0m', `Pool Address: ${poolAddr}`);
	// console.log(
	// 	"\x1b[36m%s\x1b[0m",
	// 	`Mock VDOT contract deployed to ${mockVDotAddress}`
	// );
	// console.log(
	// 	"\x1b[36m%s\x1b[0m",
	// 	`Mock VGLMR contract deployed to ${mockVGLMRAddress}`
	// );
	// console.log(
	// 	"\x1b[36m%s\x1b[0m",
