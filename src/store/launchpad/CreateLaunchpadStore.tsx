	...
	devtools(
		persist(
			(set, get) => ({
				...
				setProjectTokenAddress: (address) =>
					set({ projectTokenAddress: address }),
				...
				setMaxStakePerInvestor: (amount) =>
					set({ maxStakePerInvestor: amount }),
				setMinStakePerInvestor: (amount) =>
					set({ minStakePerInvestor: amount }),
				...
				validateForm: () => {
					...
					if (!state.selectedStakingToken)
						errors.push('Staking token is required')
					...
					// Logical validations
					if (
						Number(state.minStakePerInvestor) >
						Number(state.maxStakePerInvestor)
					) {
			...
			}),
			{
				...
				partialize: (state: LaunchpadState) => ({
					projectTokenAddress: state.projectTokenAddress,
					tokenSupply: state.tokenSupply,
					selectedStakingToken: state.selectedStakingToken,
					selectedStakingTokenSymbol: state.selectedStakingTokenSymbol,
					maxStakePerInvestor: state.maxStakePerInvestor,
					minStakePerInvestor: state.minStakePerInvestor,
